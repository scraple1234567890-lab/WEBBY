...
const profileBadgesSection = el("profileBadgesSection");
const profileBadgesGrid = el("profileBadgesGrid");
const profileBadgesCount = el("profileBadgesCount");
const profileBadgesTotal = el("profileBadgesTotal");
const profileBadgesEmpty = el("profileBadgesEmpty");
...
function setProfileBadgesVisible(show) {
  setVisible(profileBadgesSection, show);
}

function renderProfileBadges() {
  // Uses localStorage-backed achievements (per device/browser)
  ...
  // If the badge system isn't present on this build, just hide quietly
  if (!api || !Array.isArray(defs) || defs.length === 0) {
    setProfileBadgesVisible(false);
    return;
  }
  ...
  if (profileBadgesCount instanceof HTMLElement) {
    profileBadgesCount.textContent = String(unlockedDefs.length);
  }
  if (profileBadgesTotal instanceof HTMLElement) {
    profileBadgesTotal.textContent = String(defs.length);
  }

  profileBadgesGrid.innerHTML = "";

  if (!unlockedDefs.length) {
    setVisible(profileBadgesEmpty, true);
    return;
  }
  setVisible(profileBadgesEmpty, false);

  // Show most recent unlocks first (when available)
  unlockedDefs.sort((a, b) => {
    const sa = api.isUnlocked(a.id) ? 0 : 1;
    const sb = api.isUnlocked(b.id) ? 0 : 1;
    return sa - sb;
  });
...
function renderProfile(user) {
  ...
  // Render client-side badges (stored per browser)
  renderProfileBadges();
...
function init() {
  ...
  // Re-check badges when returning to this tab
  window.addEventListener("focus", () => {
    try { renderProfileBadges(); } catch {}
  });
