...
async function handleSignup(event) {
  ...
  try {
    ...
    setStatus(authStatus, error?.message || "Signup failed.", "error");
    return;
  }
...
async function handleLogout() {
  ...
  try {
    const { error } = await withTimeout(
      ...
      AUTH_TIMEOUT_MS,
      "Logout is taking too long. Please refresh and try again.",
    );
    if (error) {
...
async function loadSession() {
  try {
    ...
    currentSession = null;
    setStatus(authStatus, error?.message || "Unable to check session.", "error");
    updateAuthVisibility(false);
  }
}
