...
let sessionLoadRetriesRemaining = 2;
...
async function refreshPosts() {
  ...
  try {
    ...
    // If we're navigating away, aborted fetches are expected.
    if (isPageLeaving && isAbortLikeError(error)) return;
    feed.innerHTML = "";
...
async function handleLogin(event) {
  ...
  try {
    ...
    // Transient aborts can happen depending on the browser/network.
    if (isAbortLikeError(err)) {
      try {
        signInResult = await supabase.auth.signInWithPassword({ email, password });
      } catch (err2) {
        setStatus(loginSuccess, "");
        setStatus(loginError, "Network hiccup while signing in. Please try again.", "error");
      ...
      setStatus(loginSuccess, "");
      setStatus(loginError, err?.message || "Login failed.", "error");
      setStatus(authStatus, "Sign-in failed. Please try again.", "error");
      setLoginButtonState(false);
      return;
    }
...
async function loadSession() {
  try {
    const { data } = await getSessionSafe({ retries: 2, retryDelayMs: 75 });
    ...
    if (isPageLeaving && isAbortLikeError(error)) return;
    if (isAbortLikeError(error) && sessionLoadRetriesRemaining > 0) {
      sessionLoadRetriesRemaining -= 1;
      // Quietly retry shortly. This avoids noisy console spam on transient aborts.
      setTimeout(() => {
        loadSession();
      }, 150);
      return;
    ...
    currentSession = null;
    setStatus(authStatus, "Unable to check session. Please refresh and try again.", "error");
    updateAuthVisibility(false);
    refreshPosts();
  }
}
