...
async function refreshAdminState(session) {
  ...
  // Admin is determined server-side by SQL function public.is_admin()
  // (backed by profiles.is_admin, protected by RLS).
  try {
    const { data, error } = await supabase.rpc("is_admin");

    if (error) {
      // If the function/table isn't set up yet, stay non-admin.
    ...
    ensureAdminBar();
    setAdminBarVisible(state.isAdmin, state.isAdmin ? (state.usingDatabase ? "" : "(Run DB setup to enable editing)") : "");
    renderProducts();

    if (!state.isAdmin) {
      console.info("Shop admin editing disabled: this account is not marked as admin.");
    }
  } catch (error) {
...
async function refreshProductsFromDatabase() {
  ...
  try {
    ...
    if (Array.isArray(data)) {
      state.products = data.map(normalizeProductRow);
