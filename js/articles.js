...
function ensureArticleModal() {
  ...
  overlay.className = "modalOverlay articleModalOverlay";
  overlay.setAttribute("role", "dialog");
  overlay.setAttribute("aria-modal", "true");
  overlay.setAttribute("aria-hidden", "true");
  overlay.setAttribute("aria-labelledby", "article-modal-title");
  overlay.hidden = true;

  overlay.innerHTML = `
    <div class="modalDialog articleModalDialog" role="document">
      <div class="modalHeader">
        <div class="modalTitleWrap">
          <p class="kicker" style="margin:0;">Article</p>
          <h2 class="modalTitle" id="article-modal-title" style="margin:0;"></h2>
          <p class="muted small modalMeta" id="article-modal-meta" style="margin: 0.4rem 0 0;"></p>
        </div>
        <button type="button" class="btn ghost btnSm modalClose" id="article-modal-close" aria-label="Close article">
          Close
        </button>
      </div>

      <div class="articleModalCover" id="article-modal-cover" hidden>
        <img alt="" loading="lazy" />
      </div>

      <div class="tagList" id="article-modal-tags" hidden></div>

      <div class="articleFull articleModalBody" id="article-modal-body"></div>
    </div>
  `;
  ...
  const dialog = overlay.querySelector(".modalDialog");
  ...
  overlay.addEventListener("click", (event) => {
    if (event.target === overlay) closeIfOpen();
  });
...
function openArticleModal(article) {
  const overlay = ensureArticleModal();
  const dialog = overlay.querySelector(".modalDialog");
  const titleEl = overlay.querySelector("#article-modal-title");
  const metaEl = overlay.querySelector("#article-modal-meta");
  const bodyEl = overlay.querySelector("#article-modal-body");
  const coverWrap = overlay.querySelector("#article-modal-cover");
  const coverImg = coverWrap?.querySelector("img");
  const tagsWrap = overlay.querySelector("#article-modal-tags");
  const closeBtn = overlay.querySelector("#article-modal-close");
  ...
  if (metaEl) metaEl.textContent = `By ${authorName} • ${formatDateOnly(article?.created_at)}`;

  const fullHtml = sanitizeHtml(article?.content || "");
  if (bodyEl) bodyEl.innerHTML = fullHtml || '<p class="muted">—</p>';

  const coverUrl = article?.cover_image_url || article?.cover_url || article?.image_url || article?.image;
  if (coverUrl && coverWrap && coverImg) {
...
function isInteractiveTarget(target) {
  ...
  if (!el) return false;
  return Boolean(el.closest("button, a, input, textarea, select, details, summary"));
}
...
function renderArticles(articles) {
  ...
  articles.forEach((article) => {
    ...
    const safeTitle = article.title || "Untitled article";
    card.setAttribute("aria-label", `Open article: ${safeTitle}`);
    ...
    const excerptLimit = coverUrl ? 160 : 220;
    const excerpt = text.length > excerptLimit ? `${text.slice(0, excerptLimit).trim()}…` : text;
    ...
    hint.className = "muted small articleOpenHint";
    hint.textContent = "Click to open";

    body.append(excerptEl, hint);
