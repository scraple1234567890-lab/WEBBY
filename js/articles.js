...
async function insertInlineImages(files) {
  ...
  for (const file of files) {
    ...
    const { dataUrl } = await compressImage(file, { maxWidth: 1100, maxHeight: 1100, quality: 0.84 });
    const html = `<figure class="richFigure size-md align-center"><img src="${dataUrl}" alt="" loading="lazy"></figure>`;
    insertHtml(html);
  }
}
...
async function tryUploadCover(blob, filename) {
  try {
    ...
    const path = `${currentUser?.id || "anon"}/${Date.now()}-${filename || "cover"}.jpg`;
    ...
    console.warn("Cover upload failed, falling back to embedded image.", error);
    return null;
  }
}
