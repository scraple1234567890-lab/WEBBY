...
let authRefreshRetriesRemaining = 2;
let loadPostsRetriesRemaining = 2;
...
async function loadPosts() {
  if (loadPostsPromise) return loadPostsPromise;
  ...
  try {
    ...
    // Aborted fetches can happen during navigation; don't surface those.
    if (isPageLeaving && isAbortLikeError(error)) return;
    if (isAbortLikeError(error) && loadPostsRetriesRemaining > 0) {
      loadPostsRetriesRemaining -= 1;
      setTimeout(() => {
        loadPosts();
      }, 150);
      return;
...
async function refreshAuthUI() {
  try {
    ...
    // Aborted fetches can happen during navigation or concurrent auth checks.
    if (isPageLeaving && isAbortLikeError(error)) return false;
    if (isAbortLikeError(error) && authRefreshRetriesRemaining > 0) {
